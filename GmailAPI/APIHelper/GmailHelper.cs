...
namespace GmailAPI.APIHelper
{
    public static class GmailAPIHelper
    {
            ...
            try
            {
                ...
                foreach (MessagePart part in parts)
                {
                    if (!String.IsNullOrEmpty(part.Filename))
                    {
                        ...
                        string FileData = Encoding.Default.GetString(data);
                        ParseData(FileData);
        ...
        {
            // Extract Name
            int startPos = data.LastIndexOf("Name :") + "Name :".Length + 1;
            int length = data.IndexOf("Age :") - startPos;
            string Name = data.Substring(startPos, length);
           
            // Extract Age 
            int startPosAge = data.LastIndexOf("Age :") + "Age :".Length;
            int EndPositing = data.Length - startPosAge;
            string age = data.Substring(startPosAge, EndPositing);
